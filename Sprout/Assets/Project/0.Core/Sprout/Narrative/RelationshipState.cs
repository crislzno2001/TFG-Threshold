...
namespace Sprout.Domain.Narrative
{
    ...
    public class RelationshipState
    {
        ...
        {
            int v = Get(npc);
            if (v <= -4) return "seems to resent you";
            if (v <= -2) return "is guarded around you";
            if (v < 1)  return "is politely distant";
            if (v < 3)  return "seems to enjoy your company";
            if (v < 5)  return "trusts you";
