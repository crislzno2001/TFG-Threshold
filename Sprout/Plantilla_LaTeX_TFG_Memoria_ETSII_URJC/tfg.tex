...
El sistema de persistencia de \textit{Sprout} distingue entre dos tipos de información que cumplen funciones distintas dentro del proyecto. Por un lado se encuentra la configuración de usuario, formada por preferencias sencillas como volumen, idioma o ajustes de presentación. Este tipo de dato encaja con un almacenamiento de clave-valor, por lo que puede resolverse mediante \textit{PlayerPrefs}, mecanismo proporcionado por Unity para conservar valores simples entre sesiones.
Por otro lado se encuentra el  guardado de partida, que no almacena preferencias, sino el estado completo de una sesión de juego. Esta segunda necesidad requiere una estructura propia, ya que debe reconstruir no solo valores aislados, sino el estado narrativo, conversacional y espacial de la partida.

La diferencia entre ambos sistemas es importante porque evita mezclar datos de naturaleza distinta. La configuración describe cómo quiere jugar la usuaria; el guardado de partida describe qué ha ocurrido dentro del mundo de \textit{Sprout}. Por ello, el progreso del juego se gestiona mediante la clase \texttt{SaveSystem}, que serializa el estado relevante a un fichero JSON local llamado \texttt{sprout\_save.json}, ubicado en \texttt{Application.persistentDataPath}. Para esta serialización se utiliza \texttt{JsonUtility}, la utilidad de Unity para convertir objetos serializables a formato JSON.

El objeto que se escribe en disco es una instancia interna de \texttt{SaveData}. Esta clase agrupa todos los datos necesarios para reconstruir una partida: el día actual, la fase del día, las banderas narrativas, los contadores, el inventario de flores, el inventario de ramos, las relaciones con los NPCs, la memoria conversacional de cada personaje, el nodo actual de cada conversación, los objetivos ya completados en la fase actual y la posición y rotación de la jugadora. De este modo, el guardado no se limita a conservar recursos obtenidos, sino que permite retomar la partida en un estado narrativo y conversacional coherente.

\begin{table}[h]
\centering
\caption{Información persistida por el sistema de guardado de partida.}
\label{tab:save_system_data}
{\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{4cm}p{9.5cm}}
\hline
\textbf{Dato guardado} & \textbf{Función dentro de la partida} \\
\hline
\texttt{day} y \texttt{phase} & Permiten restaurar el momento del ciclo diario en el que se encontraba la jugadora. \\
\hline
\texttt{flags} y \texttt{counters} & Conservan el estado narrativo acumulado: decisiones tomadas, eventos activados y contadores de progreso. \\
\hline
\texttt{flowers} y \texttt{bouquets} & Guardan el inventario de flores emocionales y ramos creados. \\
\hline
\texttt{relationships} & Mantienen el estado de relación con cada NPC. \\
\hline
\texttt{memories} & Guardan los recuerdos persistentes de cada NPC sobre la jugadora. \\
\hline
\texttt{currentNodes} & Permiten retomar cada conversación desde el nodo exacto en el que quedó. \\
\hline
\texttt{phaseGoals} & Conservan qué objetivos de la fase actual ya se habían completado. \\
\hline
\texttt{playerPos} y \texttt{playerRot} & Restauran la posición y orientación de la jugadora al continuar la partida. \\
\hline
\end{tabular}
}
\end{table}

Una decisión técnica relevante es que el fichero JSON no guarda los grafos de diálogo completos. Los grafos son assets de autoría creados con \texttt{ScriptableObject}: contienen los nodos, sus conexiones, las condiciones narrativas y el contexto para la IA. Esos datos forman parte del proyecto y no deben modificarse como consecuencia de una partida concreta. El guardado, en cambio, almacena únicamente el estado alcanzado por la jugadora dentro de esos grafos. Por esta razón, cuando se guarda una conversación, no se serializa el nodo entero, sino el identificador \texttt{nodeGuid} del nodo actual asociado al nombre del NPC. Al cargar la partida, el sistema busca ese identificador dentro del grafo correspondiente y restaura el \texttt{DialogueRunner} en ese punto.

Esta separación evita dos problemas. En primer lugar, impide que el progreso de una partida modifique accidentalmente los assets de diálogo usados como mapa narrativo. En segundo lugar, reduce la duplicación de datos: el JSON no necesita repetir toda la estructura de los grafos, sino solo recordar en qué punto de cada grafo se encontraba la partida. Así, los grafos funcionan como definición estable de la historia, mientras que el fichero de guardado registra la trayectoria concreta de la jugadora.

El método \texttt{Save()} construye el objeto \texttt{SaveData} a partir del estado actual del juego. Primero recupera el día y la fase desde el director de juego. Después exporta el progreso de fase desde \texttt{DayCycleService}, de modo que al continuar no se reinicien los NPCs ya completados en la fase actual. A continuación almacena la posición de la jugadora, las banderas narrativas, los contadores, las flores, los ramos y las relaciones. Para cada NPC registrado, el sistema exporta además su memoria conversacional mediante \texttt{ExportMemory()} y guarda el nodo actual de su \texttt{DialogueRunner}, siempre que exista un identificador válido.

Dado que parte del estado interno se gestiona mediante diccionarios, el sistema lo transforma antes de serializarlo. En lugar de guardar directamente un diccionario, se utilizan clases auxiliares serializables como \texttt{Pair}, \texttt{FlagPair}, \texttt{MemEntry} y \texttt{NodeEntry}. Por ejemplo, una entrada de memoria se guarda como una terna formada por el nombre del NPC, la clave del recuerdo y su valor. Esta conversión hace explícita la estructura del fichero JSON y evita depender de una serialización directa de colecciones que \texttt{JsonUtility} no maneja de forma general.

El proceso de carga realiza la operación inversa. El método \texttt{Load()} lee el fichero JSON, reconstruye el objeto \texttt{SaveData} y vuelve a convertir las listas serializadas en diccionarios de uso interno. Después restaura las banderas y contadores narrativos, el inventario de flores y ramos, las relaciones con NPCs y el estado del ciclo diario. La memoria conversacional se agrupa por nombre de NPC y se reinyecta en cada \texttt{NPCBrain} mediante \texttt{ImportMemory()}. De esta forma, cada personaje recupera los recuerdos que tenía sobre la jugadora antes de cerrar la partida.

La restauración del nodo actual de conversación sigue el mismo principio. Cada entrada guardada contiene el nombre del NPC y el \texttt{nodeGuid} del nodo en el que se encontraba su conversación. Durante la carga, el sistema localiza el \texttt{NPCBrain} correspondiente, busca dentro de su \texttt{dialogueGraph} el nodo con ese identificador y llama a \texttt{RestoreCurrent()} en el \texttt{DialogueRunner}. Gracias a este mecanismo, una conversación puede reanudarse desde el punto exacto en el que quedó, sin convertir el grafo en parte del fichero de guardado.

Por último, si el fichero contiene posición de la jugadora, el sistema localiza el objeto con la etiqueta configurada, desactiva temporalmente su \texttt{CharacterController}, aplica la posición y rotación guardadas, y vuelve a activar el controlador. Esta operación evita conflictos físicos durante el reposicionamiento y permite que la jugadora continúe desde el lugar donde dejó la partida, en lugar de reaparecer siempre en un punto fijo.

En la versión actual, el guardado no se ejecuta automáticamente al producirse un cambio de fase. Aunque \texttt{SaveSystem} se suscribe al evento de cambio de fase, el método asociado no llama a \texttt{Save()}; el propio código indica que el guardado se realiza cuando otro sistema lo invoca explícitamente, por ejemplo al dormir. Por tanto, la forma más precisa de describirlo no es como un guardado automático al llegar la noche, sino como un guardado de partida ejecutado en el momento de descanso o cierre de ciclo definido por el flujo jugable.
